...
#  -DInput=<path/to/input/matrix> -DOutput=<path/to/outputfolder> -DRows=<number of rows> -DCols=<number of columns> -DPCs=<number of principal components> [-DErr=<error sampling rate>] [-DMaxIter=<max iterations>] [-DComputeProjectedMatrix=<1/0>] [-DOutFmt=<output format>] 
...
$SPARK_HOME/bin/spark-submit --class org.qcri.sparkpca.SparkPCA --master $master_url --driver-java-options "-DInput=$SCRIPTPATH/input/seqfiles -DOutput=$SCRIPTPATH/output/ -DRows=7 -DCols=5 -DPCs=3 -DErr=1 -DMaxIter=3" $SCRIPTPATH/target/sparkPCA-1.0.jar 
